...
DEFAULT_LABELS_PATH = SCRIPT_DIR / "data" / "imagenet32" / "train_y.npy"
...
def summarize_subset(
    ...
    *,
    ...
    absolute_distribution_error = np.abs(subset_distribution - full_distribution)

    # Total variation distance is zero for identical distributions and
    # approaches one as distributions diverge.
    total_variation_distance = float(0.5 * absolute_distribution_error.sum())
    ...
    return {
        ...
        "maximum_class_share_error": float(absolute_distribution_error.max()),
...
def print_report(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        class_display = f"{row['classes_in_subset']}/{row['classes_in_full_dataset']}"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=("Validate deterministic stratified subsets without training.")
    )
...
def main() -> None:
    ...
    for fraction in args.fractions:
        if not 0.0 < fraction <= 1.0:
            raise ValueError(f"Every fraction must be in (0, 1], got {fraction}.")

    if args.seed == args.compare_seed:
        raise ValueError("--seed and --compare-seed must be different.")
    ...
    for fraction in sorted(args.fractions):
        primary_indices, metadata, manifest_path = get_or_create_stratified_indices(
            labels,
            fraction=fraction,
            seed=args.seed,
            split="train",
        ...
        alternate_indices, _, _ = get_or_create_stratified_indices(
            labels,
            fraction=fraction,
    ...
    csv_path = args.output_dir / f"{report_prefix}_subset_report_seed{args.seed}.csv"

    json_path = args.output_dir / f"{report_prefix}_subset_report_seed{args.seed}.json"
